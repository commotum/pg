...
class Hyperparameters:
    ...
    quat_mlp_impl = os.environ.get("QUAT_MLP_IMPL", "matrix").lower()
...
def _quaternion_matrix(w):
    wr, wi, wj, wk = w.unbind(dim=0)
    top = torch.cat([wr, -wi, -wj, -wk], dim=1)
    row_i = torch.cat([wi, wr, -wk, wj], dim=1)
    row_j = torch.cat([wj, wk, wr, -wi], dim=1)
    row_k = torch.cat([wk, -wj, wi, wr], dim=1)
    return torch.cat([top, row_i, row_j, row_k], dim=0)


def _quaternion_input_hessian(x):
    if x.ndim == 3:
        x = x.reshape(-1, x.shape[-1])
    if x.shape[-1] % 4 != 0:
        raise ValueError(f"qMLP dimension must be divisible by 4, got {x.shape[-1]}")
    xq = x.reshape(-1, 4, x.shape[-1] // 4).float()
    h = torch.zeros(xq.shape[-1], xq.shape[-1], dtype=torch.float32, device=x.device)
    for part in xq.unbind(dim=1):
        h.addmm_(part.T, part)
    return h


_QUAT_COMPONENTS = ("wr", "wi", "wj", "wk")


def _is_quat_mlp_component(name):
    return ".mlp." in name and name.rsplit(".", 1)[-1] in _QUAT_COMPONENTS
...
class MLP(nn.Module):
    ...
    def forward(self, x, up_w, down_w):
        if up_w.ndim == 3:
            up_w = _quaternion_matrix(up_w).to(x.dtype)
            down_w = _quaternion_matrix(down_w).to(x.dtype)
        else:
        ...
        hidden = F.leaky_relu(F.linear(x, up_w), negative_slope=0.5).square()
...
class GPT(nn.Module):
    def __init__(self, h):
        ...
        self.quat_mlp = bool(h.quat_mlp)
        if self.quat_mlp and h.quat_mlp_impl != "matrix":
            raise ValueError(f"unsupported QUAT_MLP_IMPL={h.quat_mlp_impl!r}")
        head_dim = h.model_dim // h.num_heads
        kv_dim = h.num_kv_heads * head_dim
        hidden_dim = int(h.mlp_mult * h.model_dim)
        if self.quat_mlp and (h.model_dim % 4 != 0 or hidden_dim % 4 != 0):
            raise ValueError(
                f"qMLP requires model_dim and hidden_dim divisible by 4, got "
                f"{h.model_dim} and {hidden_dim}"
            )
        ...
        if self.quat_mlp:
            self.mlp_up_bank = nn.Parameter(
                torch.empty(h.num_layers, 4, hidden_dim // 4, h.model_dim // 4)
            )
            self.mlp_down_bank = nn.Parameter(
                torch.empty(h.num_layers, 4, h.model_dim // 4, hidden_dim // 4)
            )
        else:
    ...
    def _init_weights(self):
        ...
        for i in range(n):
            if self.quat_mlp:
                for c in range(4):
                    nn.init.orthogonal_(self.mlp_up_bank.data[i, c], gain=1.0)
...
def collect_hessians(model, train_loader, h, device, n_calibration_batches=64):
    ...
    def make_mlp_hook(layer_idx):
        def hook_fn(module, inp, out):
            ...
            if x.ndim == 3:
                x = x.reshape(-1, x.shape[-1])
            if getattr(model, "quat_mlp", False):
                hq = _quaternion_input_hessian(x)
                for suffix in _QUAT_COMPONENTS:
                    name = f"blocks.{layer_idx}.mlp.fc.{suffix}"
                    if name not in hessians:
                        hessians[name] = torch.zeros_like(hq)
                    hessians[name].add_(hq)
            else:
            ...
            if h_act is not None:
                ...
                if getattr(model, "quat_mlp", False):
                    hq = _quaternion_input_hessian(h_act)
                    for suffix in _QUAT_COMPONENTS:
                        name = f"blocks.{layer_idx}.mlp.proj.{suffix}"
                        if name not in hessians:
                            hessians[name] = torch.zeros_like(hq)
                        hessians[name].add_(hq)
                else:
...
def gptq_mixed_quantize(state_dict, hessians, h):
    ...
    for (name, tensor) in state_dict.items():
        ...
        force_quantize = _is_quat_mlp_component(name)
        ...
        if (
            ...
            continue
        if not force_quantize and (not t.is_floating_point() or t.numel() <= 65536):
...
_GROUP_ORDER = [
    ...
    "mlp.fc.wr.q", "mlp.fc.wi.q", "mlp.fc.wj.q", "mlp.fc.wk.q",
    "mlp.proj.wr.q", "mlp.proj.wi.q", "mlp.proj.wj.q", "mlp.proj.wk.q",
]
_SIMSORT_KEYS = {
    "_tok_emb.weight.q",
    "attn.c_q.weight.q",
...
def _unbank_state_dict(state_dict, num_layers):
    ...
    for k, v in state_dict.items():
        ...
        if k == "qo_bank":
            ...
            for i in range(n):
                if t.ndim == 4:
                    for c, suffix in enumerate(_QUAT_COMPONENTS):
                        sd[f"blocks.{i}.mlp.fc.{suffix}"] = t[i, c]
                else:
            ...
            for i in range(n):
                if t.ndim == 4:
                    for c, suffix in enumerate(_QUAT_COMPONENTS):
                        sd[f"blocks.{i}.mlp.proj.{suffix}"] = t[i, c]
                else:
...
def _rebank_state_dict(flat_sd, num_layers, model_dim, kv_dim, hidden_dim, quat_mlp=False):
    sd = {}
    n = num_layers
    ...
    if quat_mlp:
        sd["mlp_up_bank"] = torch.zeros(n, 4, hidden_dim // 4, model_dim // 4)
        sd["mlp_down_bank"] = torch.zeros(n, 4, model_dim // 4, hidden_dim // 4)
        for i in range(n):
            for c, suffix in enumerate(_QUAT_COMPONENTS):
                sd["mlp_up_bank"][i, c] = flat_sd[f"blocks.{i}.mlp.fc.{suffix}"]
                sd["mlp_down_bank"][i, c] = flat_sd[f"blocks.{i}.mlp.proj.{suffix}"]
    else:
...
def deserialize(h, device):
    ...
    deq_state = _rebank_state_dict(
        deq_flat, h.num_layers, h.model_dim, kv_dim, hidden_dim, quat_mlp=h.quat_mlp
    )
...
def train_model(h, device, val_data):
    ...
    log(f"quat_mlp:{h.quat_mlp}")
    log(f"quat_mlp_impl:{h.quat_mlp_impl}")
